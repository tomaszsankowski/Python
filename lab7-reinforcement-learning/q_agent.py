...
class QAgent(Agent):
    ...
    def __init__(self, n_states, n_actions,
        ...
        self.epsilon = math.exp(-1)           # epsilon (p-wo akcji losowej)
        self.eps_decrement = math.exp(-4)     # wartość, o którą zmniejsza się epsilon po każdym kroku
        self.eps_min = math.exp(-3)           # końcowa wartość epsilon, poniżej którego już nie jest zmniejszane
    ...
    def learn(self, state: State, action: Action, reward: float, new_state: State, done: bool) -> None:
        ...
        new_experience = reward + self.gamma * np.max(self.q_table[new_state])
        knowledge = self.q_table[state][action]
        self.q_table[state][action] = knowledge + self.lr * (new_experience - knowledge)
