...
class Node:
    ...
    def gini_best_score(self, y, possible_splits):
        ...
        for split_id in possible_splits:

            left_pos = np.sum(y[:split_id+1] == 1)
            left_neg = np.sum(y[:split_id+1] == 0)
            right_pos = np.sum(y[split_id+1:] == 1)
            right_neg = np.sum(y[split_id+1:] == 0)
            ...
            gini = 1 - gini_left * left / (left + right) - gini_right * right / (left + right)
